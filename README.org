...
Credit Check is a free tool from WikiPortraits for Wikimedia Commons photographers. It searches Wikipedia to find the articles that feature your photos, then adds those photos (or a subset) to your category page on Wikimedia Commons — using a category name like =Photographs by Jay Dixit= — enabling you to easily keep track of which Wikipedia articles use your photos.
...
After Credit Check finds your photos, it opens a photo picker in a browser. You can then select which photos to add, return to the CLI app, and add them.
...
Credit Check was built by [[https://jaydixit.com/][Jay Dixit]] ([[mailto:jay@wikiportraits.org][jay@wikiportraits.org]]).
...
It opens a contact sheet at =127.0.0.1= with Wikimedia Commons thumbnails, category sections, search, review modes (=All=, =Selected=, and =Not selected yet=), select-shown/select-all buttons, and links that open each file on Wikimedia Commons. Selected photos get a visible badge and left stripe so you can scan the page quickly. The =Commons edits= panel updates automatically with the exact category edits for the photos you picked. The page saves your choices automatically. Use =Done= when you are finished.
