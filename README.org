#+TITLE: Credit Check
#+SUBTITLE: Find Commons files you photographed that are used on Wikipedia
...
That is the main workflow. If you also provide a subject category and Wikidata
QID, Credit Check can put photos of you taken by other people in a separate
review section.

Run =credit-check= to start the guided command-line app. Credit Check writes a
plain Markdown review file first, opens a local browser contact sheet so you
can pick photos visually, and edits Commons only when you confirm =commit
--go=. Photos of you taken by other people are a separate guided action.
...
4. *Sorts you from subjects.* Photos you took go one way. Photos that merely
   depict you (shot by someone else) never land in your "Photographs by"
   category, and can go to your own subject category instead.
5. *Traces crops.* If a crop dropped your credit but still cites your original
   file as its source, Credit Check follows the trail and claims it anyway.
6. *Waits for your yes.* Nothing is edited until you tick a box.
...
From this checkout, install it as an editable command with pipx:

: cd path/to/credit-check
: pipx install --editable .

Now =credit-check= runs from anywhere, and it uses the code in this repo while
you iterate.
...
credited name, and your photographer category. After that, it shows the current
review status and puts the natural next action first: find your photos, review
found photos, or add the photos you picked. If you add your Wikidata ID in
Settings, a separate optional action appears for photos of you taken by other
people.

In guided mode, Credit Check uses the local review file automatically. After a
scan, it opens the browser review instead of asking you which file to use. The
Settings action saves your Commons username, credited name, and categories in
[[file:.credit-check.json][.credit-check.json]] for this folder, so you do not have to re-enter them.
...
: export WIKI_OF_CATEGORY='Jay Dixit'                  # subject category, for photos that depict you
...
- =source_depth= controls crop tracing. A hop is one step from a crop back to
  ...
  without making the scan wander too far.
...
sections, search, visible review modes (=All=, =Selected=, and =Still deciding=),
select-visible/select-all buttons, and links that open each file on Commons.
Selected photos get a visible badge and left stripe so you can scan the page
quickly. Use =Preview edits= to see the exact category edits for the photos
you picked before you leave the browser. The page saves your choices back into
=review.md= and shows the next command after saving. Use =Save and close= when
you are done.
...
- =g= opens the current page as a browser contact sheet.
- =v= opens the full review as a browser contact sheet.
...
- Files under =# Ambiguous= are never edited automatically. To categorize one,
  move it up under a real heading, then tick it.
...
You can also preview the same edits inside the browser review with
=Preview edits=.
...
unchecked photos remain in =review.md=.
...
| Option           | What it does                                     |
|------------------+--------------------------------------------------|
| =--go=           | Actually edit (default is a dry run)             |
| =--throttle S=   | Seconds between edits (default 5)                |
...
| =review --selected REVIEW= | Opens the browser on your selected photos |
...
By default Credit Check ignores photos of you: a portrait someone else shot
won't be filed under "Photographs by you." In the guided app, choose =Find
photos of you by other people= after adding your Wikidata ID in Settings. If you
use direct commands and have a Wikidata item, point the tool at it:
...
Credit Check is built to be a considerate editor, and you should run it like
one:

- Nothing is edited unless you pick it. Dry run is the default.
...
- It throttles between edits and backs off while fetching or logging in if
  Commons asks it to slow down.
- It does not blindly retry edit submissions after a lost network response; it
...
- *"Missing --username (or WIKI_USERNAME)"*: set your identity (see Setup).
...
It won't upload files, edit Wikipedia articles, rewrite your metadata, add
event or location categories, or edit anything without your say-so. Its job is
narrow on purpose: get your Wikipedia-used photos into your photographer
category.
...
makes sure it shows up in your category.
