...
The report counts views of Wikipedia articles carrying your photographs, rather than views of the image files. Shared articles count once in the overall total. If the pageview service is unavailable or returns only some results, the report says so; missing measurements are never presented as measured zeroes.
...
Credit Check already knows every Wikipedia article that carries each photo, so this optional step asks the Wikimedia pageviews API for those articles and reports their combined reach. It counts views of the Wikipedia articles that carry your photos, not requests for the image files themselves. That is the standard GLAM meaning behind wording such as photos that "reach" a given number of views.
...
: Views: 4,120,338 last month (2026-07), 38,207,911 over 12 months, across 1,429 articles (3 no data, 0 failed).

Overall totals count each distinct Wikipedia article once, even if it carries more than one of your photos. In the per-photo top ten, however, an article that carries two of your photos contributes its full article views to both photos. The browser reach ledger shows the last complete month, the full window, monthly totals, and links to the ten photos with the most article views in the last month.
