...
[[file:CHANGELOG.org][Changelog — new features, changes, and fixes]]

Credit Check gives you a way to see where all your photos are being used on Wikipedia and Wikidata.
...
3. *Shows Wikidata uses.* For those photos, finds the Wikidata items that use each image and resolves their English labels.
4. *Spots the gaps.* Flags the images missing =Category:Photographs by <you>=.
5. *Traces cropped versions of your photos.* If a crop dropped your credit but still cites your original file as its source, Credit Check follows the trail back to you and includes it too.
6. *You select* which photos you want to add to your photographer category.
...
#+ATTR_HTML: :alt Credit Check browser gallery showing Wikipedia reach, a Wikimedia Commons photographer category, and photos of Sydney Sweeney, Bong Joon Ho, and Alexander Skarsgård :width 100%
[[file:assets/credit-check-gallery.jpeg]]

When you're done, you can also start over with a different photographer.

* Share your Wikipedia reach

Choose ~Create a reach report~ in the guided app to export your complete photo gallery, its Wikipedia articles and language editions, and changes since your previous matching scan. You can optionally include the last 12 complete months of image-file requests and article pageviews. The HTML report opens in your browser and can be saved as a PDF using Print. It includes links to the photographs and the articles that use them.

Image requests are actual requests for your image files across all referring sites. They use user-classified traffic, which excludes identified automated traffic, but may include preloads; they are not unique people or verified visual impressions. Totals sum the measurements that are available, while separate coverage shows how many photos were measured in each month and how many have complete histories. Article views remain secondary context: they show past views of Wikipedia articles carrying your photos at scan time, and your photos may not have been on those pages for the whole reporting period. Shared articles count once in the overall total. Your Main Page placements stay listed but are excluded only from article-view totals because a brief appearance can inflate them. If either service is unavailable or returns only some results, the report says so; missing measurements are omitted rather than presented as measured zeroes.

Each new scan keeps a dated snapshot and preserves the previous review before replacing it. Comparisons use the same photographer and scan settings, so a filter change cannot masquerade as newly added or removed placements. Older saved reviews without a recorded scan date retain that uncertainty. Local history and reports stay on your computer.

Direct commands are available too:

#+begin_src sh
credit-check report
credit-check report --views --views-months 12
credit-check report --format json --out /full/path/reach-report.json
credit-check report --format csv --out /full/path/photo-article-placements.csv
#+end_src

The CSV contains one row per photograph and article placement. When image requests were fetched, it adds the date window, traffic filters, that photograph's measurement status and month coverage, and its available request totals. A photograph used in several articles appears in several rows, so do not add those repeated image-request values together. CSVs created without =--views= keep the original four placement columns. Existing export files are never overwritten.
...
Since our first shoot at Sundance in 2024, our portraits have reached Wikipedia articles in over 150 languages. Project-wide reporting from GLAMorgan puts those Wikipedia articles at more than 150 million views a month. Among the events our photographers have captured: Sundance Film Festival, SXSW, Cannes Film Festival, Edinburgh Festival Fringe, Venice Film Festival, Toronto International Film Festival, the Nobel Prizes, CES, and the Pulitzer Prizes.
...
** Experimental: decide which photos to upload next

You may have thousands of photographs that could help Wikipedia, but uploading a photo of someone who already has a strong recent portrait has much less impact than replacing a missing, tiny, or decade-old lead image. Gap Check turns names or descriptively named photo files into a ranked upload queue so you can start where a new photograph matters most.

Gap Check is experimental and ready to use.

Run =credit-check= and choose =Decide which photos to upload next=. Gap Check will ask whether you want to check a folder of descriptively named photos, a text file with one name per line, or names you enter directly.

Or run it directly from a source checkout:

#+begin_src sh
python3 credit_check.py gaps --from-filenames /full/path/to/photo-folder/
python3 credit_check.py gaps /full/path/to/names.txt
python3 credit_check.py gaps "Vanessa Kirby" "Jude Law"
python3 credit_check.py gaps /full/path/to/names.txt --json
#+end_src

If Credit Check is installed, use =credit-check gaps= with the same arguments. The command recognizes common image formats, reads names from the part of each filename before the first underscore, and can split a filename such as =jason-bateman-jude-law_tiff_2025.jpg= into two people. It also uses Credit Check's saved author and Wikimedia Commons username to recognize lead images already credited to you; =--author= and =--username= override those defaults for one run.

The ranked bands are:

- =P1= --- the English Wikipedia article has no lead image.
- =P2= --- the lead image is under 1.2 megapixels.
- =P3= --- the lead image is at least six years old or lacks usable Wikimedia Commons metadata.
- =P4= --- the lead image is recent and above the resolution threshold.
- =YOURS= --- the lead image is already credited to you.
- =--= --- there is no English Wikipedia article, or the title resolves to a disambiguation page.

In one 47-name Lightroom-index run, Gap Check surfaced Jack Johnson with a 12-year-old lead image, Demetri Martin at 449 by 540 pixels, Daron Acemoglu at 486 by 648 pixels, and five additional lead images already credited to Jay. Those results can change whenever Wikipedia changes, so re-run the command before deciding what to upload.

Gap Check checks only the English Wikipedia lead image. It does not inspect other language editions or images farther down an article, and it measures resolution and age rather than photographic quality. Filename parsing is still heuristic, so qualified article titles and unusual multi-person filenames may need manual input. The command is read-only: it never uploads a photograph or edits Wikipedia or Wikimedia Commons.
...
The direct =scan= command automatically reuses the identity that the guided app saved for this folder in [[file:.credit-check.json][.credit-check.json]]. You can override any saved value with an environment variable or a command-line flag; the precedence is flag → environment → saved preference → default.

For scripts that should not depend on the current folder's saved settings, provide the identity values as flags (=--username=, =--author=, and so on) or as environment variables:
...
Direct-command users can also put advanced defaults in [[file:.credit-check.json][.credit-check.json]]. This is optional; the guided app manages ordinary identity settings for you.
...
**** Add image requests and Wikipedia article views

Add =--views= when you need a current reach figure for a credential letter, press pitch, or impact report:

: credit-check scan --views

This optional step fetches requests for each image file across all referring sites, plus past pageviews for the Wikipedia articles found in your scan. Image requests use Wikimedia's ~all-referers~ and ~user~ measurements: identified automated traffic is excluded, while browser preloads may still count. These are requests, not unique people or verified visual impressions. Totals sum only valid monthly measurements; missing months are omitted rather than counted as zero. Coverage states how many photos have any measurements, how many have complete histories, and how many were measured in the final month. A recently uploaded photo may therefore contribute its available months without being presented as a complete 12-month history. A response with no measured rows is reported as no data. Main Page placement does not exclude an image from image-request totals.

Wikipedia article views are secondary context. They describe the readership of articles carrying your photos at scan time, not how often someone saw your photos, and they do not establish that your photos were present throughout the reporting period: if a photo was added yesterday, the article's views from last month still count. Main Page placements stay listed but their article pageviews are excluded, because a single day on a Main Page can add millions of views. Credit Check asks each Wikipedia for its own Main Page title; if that lookup fails, views from that Wikipedia are deferred and the report says so. A photo used only on Main Pages shows its image requests when available while its article views remain excluded. Measurements saved by earlier versions stay hidden until you create a new reach report with ~--views~.

The default window is the 12 complete calendar months ending with the last complete UTC month. Use =--views-months N= to request from 1 through 24 months. The current partial month is never included.

: Image requests: 820,410 last month (2026-07), 7,441,902 across available measurements in the 12-month window. Photos measured: 207 of 210; complete histories: 198 of 210; 2026-07 coverage: 205 of 210.
: Article views: 4,120,338 last month (2026-07), 38,207,911 over 12 months, across 1,429 articles (3 no data, 0 failed).

The reach report presents image requests first and includes each photo's measured requests with its evidence. Article views appear afterward as context. Article-view totals count each distinct Wikipedia article once, even if it carries more than one of your photos. In the per-photo top ten, however, an article that carries two of your photos contributes its full article views to both photos. Do not add the per-photo article-view totals together; shared articles would be counted more than once. The browser reach ledger shows the last complete month, the full window, monthly totals, and links to the ten photos with the most article views in the last month.

Results are cached beside the review files in [[file:.credit-check-views.json][.credit-check-views.json]]. Credit Check reuses an article only when the cache contains every requested month; otherwise it refreshes that article's complete window. A normal scan without =--views= does not read, write, or report pageviews.
...
It opens a =Credit Check= photo picker at =127.0.0.1= with Wikimedia Commons thumbnails, category sections, search, review modes (=All=, =Selected=, and =Not selected yet=), and select-shown/select-all buttons. A large =Credit Check= product title and the line =See where your photographs live across Wikipedia.= anchor the upper-left. The linked =A free tool from WikiPortraits= endorsement and official WikiPortraits logo sit opposite on desktop and stack beneath the title on mobile; both open [[https://www.wikiportraits.org/][wikiportraits.org]].

The page opens with an editorial =Your reach on Wikipedia= ledger. It shows =X Photos used on Wikipedia=, =Y Wikipedia articles=, and =Z Language editions using at least one of your photos= with restrained Tabler camera, Wikipedia W, and world icons. A smaller =Beyond Wikipedia= line reports how many photos appear across how many distinct Wikidata items without competing with the primary Wikipedia totals. Immediately below, one unified workboard holds the three icon-led views and everything they control. The tabs sit on the workboard's top edge and show a concise title plus live count: =All your photos=, =Photos to add=, and =On Wikidata=. This orders the views from the complete Wikipedia collection to its actionable subset and then the related Wikidata usage, while =Photos to add= remains active by default in the review workflow. Their icons use the WikiPortraits shutter trio—green, red, and blue—while the active tab loses its bottom border and visually merges into the white content panel below. The three-tab rail belongs only to the gallery column, while the contextual rail begins alongside it at the workboard's top edge, so no empty fourth segment appears. The connected panel contains the active-mode explanation, filters, gallery, and contextual rail, making the tab-to-content relationship explicit without a decorative colored stripe. Only =On Wikidata= navigates to the read-only Wikidata gallery. All three views keep the same picker width, left edge, and control positions, so changing tabs does not make the page jump.

On desktop, three image-first editorial cards fill each row. Each begins with a full-bleed, top-biased 4:5 thumbnail, a compact badge showing its distinct Wikipedia article count, and fine green crop marks that give the gallery an editorial contact-sheet character. Opening the image shows the uncropped original on Wikimedia Commons. Beneath it, the photo's English Wikimedia Commons caption appears in Libre Caslon Text, falling back first to a useful English description and then, for the few photos without either, a cleaned filename. Under =Your photo appears in:=, the card shows up to four English Wikipedia article titles. When there are more, =+ N more · View all M English-language Wikipedia articles= identifies the hidden remainder and opens the complete English list in one wide view, where the full metadata caption is repeated. Pages in other Wikipedia languages expand separately and are grouped by language. If a photo has no English use, the card shows its first available article and names that Wikipedia language. When the same image is used on Wikidata, a restrained blue, logo-led =Also used on Wikidata · N items= disclosure opens a linked list of labeled items. Expand =Photo details= for the original filename and Wikimedia Commons link. Selected photos get a circular green check and ring.

The gallery and a clearly divided white right rail form one internal workspace. In the missing-category view, the rail begins with a quiet pale-sage photographer-category reference and its compact =View on Wikimedia Commons= link. Beneath that destination, the workflow is deliberately terse: =N photos selected=, a native =See details= disclosure for the exact category additions, a persistent =Saving...= / =All changes saved= indicator, and finally =Continue=. A right-pointing disclosure triangle turns downward and the label changes to =Hide details= while the edit list is open. =Continue= saves the current selection, closes the picker, and hands the workflow back to the terminal; both it and the details disclosure remain unavailable until at least one photo is selected. The read-only Wikipedia and Wikidata views keep the same rail column and replace the editing controls with an explanation of the active gallery. Their guidance uses the visible tab labels, and the Wikidata view removes the irrelevant missing-category metrics while changing its filter prompt to mention Wikidata items. A footer credits Jay Dixit and the WikiPortraits community. The whole viewport scrolls normally, and the layout collapses to one column on smaller screens.
